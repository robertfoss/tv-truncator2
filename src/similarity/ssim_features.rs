...
use super::common::{DtwAlignment, KeyPoint, SceneSegment};
use crate::Result;
...
fn extract_ssim_features(img: &DynamicImage) -> Result<Vec<f64>> {
    ...
    for y in 0..8 {
        for x in 0..8 {
            ...
            let variance = block_pixels.iter().map(|p| (p - mean).powi(2)).sum::<f64>() / 64.0;
...
pub fn compute_ssim(img1: &DynamicImage, img2: &DynamicImage) -> f64 {
    ...
    let resized1 = image::imageops::resize(
        &gray1,
        target_width,
        target_height,
...
pub fn dtw_align_segments(
    ...
    for direction in 0..2 {
        ...
        for shift in 0..=max_shift.min(feat_a.len().saturating_sub(1)) {
            ...
            if end_a < 10 {
                // Need at least 10 frames for a valid segment
                continue;
            }
