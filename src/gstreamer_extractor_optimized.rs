    ...
    F: Fn(usize, usize) + Send + Sync + 'static,
{
    ...
    println!("🚀 Seeking to {} specific timestamps", frame_timestamps.len());
    ...
    for (i, &timestamp) in frame_timestamps.iter().enumerate() {
        ...
        if i % 20 == 0 || i == frame_timestamps.len() - 1 {
            println!("  🚀 Frame {}/{} at {:.2}s", i + 1, frame_timestamps.len(), timestamp);
        }
    }
    ...
    println!("🚀 Breakdown: Duration={:.2}%, Pipeline={:.2}%, Seek={:.2}%", 
    ...
    F: Fn(usize, usize) + Send + Sync + 'static,
{
    ...
    println!("🚀 Starting PARALLEL GStreamer frame extraction for: {:?}", video_path);
    println!("🚀 Sample rate: {}, workers: {}", sample_rate, parallel_workers);
    ...
    println!("🚀 Processing {} frames in {} parallel chunks", frame_timestamps.len(), chunks.len());
    ...
    let all_frames: Vec<Frame> = chunks
        ...
        .map(|(chunk_idx, timestamps)| {
            println!("🚀 Worker {} processing {} timestamps", chunk_idx, timestamps.len());
