//! Test for time-shifted ending credits detection
#![allow(dead_code)]
...
#[test]
fn test_ending_credits_time_shifted_detection() -> Result<()> {
    ...
    // Verify detected segment covers the expected ending-credits interval.
    // Some detector paths merge adjacent common content and may extend earlier.
    assert!(
        segment.start_time <= 30.0,
        "Segment should include the ~25s start region, got {:.1}s",
        segment.start_time
    );
    assert!(
    ...
    assert!(
        duration >= 40.0,
        "Ending credits should be at least 40s, got {:.1}s",
        duration
    );
    ...
    for timing in episode_timings {
        ...
        if let Some(expected) = expected_timings.get(&timing.episode_name) {
            let expected_start = parse_time_string(&expected.start);
            let expected_offset = parse_time_string(&expected.offset);
            // Allow 5 second tolerance for start time (detection may find slightly different bounds)
            let start_diff = (timing.start_time - expected_start).abs();
            assert!(
                start_diff < 5.0,
                "{}: start time should be ~{:.1}s (±5s), got {:.1}s (diff: {:.1}s)",
                timing.episode_name,
                expected_start,
                timing.start_time,
                start_diff
            ...
            println!(
                ...
                detected_offset, expected_offset
            );
        }
    }
    ...
    println!("\n✓ Time-shifted ending credits detected correctly!");
