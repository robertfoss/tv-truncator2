...
use crate::Result;
use crate::analyzer::{Frame, generate_perceptual_hash};
use crate::Config;
use std::path::Path;
    ...
    F: Fn(usize, usize) + Send + Sync + 'static,
{
    ...
    println!("🚀 Starting OPTIMIZED GStreamer frame extraction for: {:?}", video_path);
    ...
    println!("🚀 Video duration: {:.2}s, expected frames: {}", duration, expected_frames);
    ...
    if final_frame_count == 0 {
        return Err(anyhow::anyhow!("GStreamer pipeline timed out or failed to produce frames"));
    }
    ...
    println!("🚀 Optimized streaming + hash generation: {:?}", streaming_time);
    println!("🚀 TOTAL OPTIMIZED EXTRACTION TIME: {:?}", total_extraction_time);
    println!("🚀 Optimized breakdown: Duration={:.1}%, Pipeline={:.1}%, Streaming={:.1}%",
    ...
    F: Fn(usize, usize) + Send + Sync + 'static,
{
    ...
    println!("🎯 Starting SEEK-BASED GStreamer frame extraction for: {:?}", video_path);
    println!("🎯 Sample rate: {}", sample_rate);
    ...
    println!("🎯 Seek-based streaming + hash generation: {:?}", streaming_time);
    println!("🎯 TOTAL SEEK-BASED EXTRACTION TIME: {:?}", total_extraction_time);
    println!("🎯 Seek-based breakdown: Duration={:.1}%, Pipeline={:.1}%, Streaming={:.1}%",
        duration_time.as_secs_f64() / total_extraction_time.as_secs_f64() * 100.0,
        pipeline_time.as_secs_f64() / total_extraction_time.as_secs_f64() * 100.0,
        streaming_time.as_secs_f64() / total_extraction_time.as_secs_f64() * 100.0);
    ...
    F: Fn(usize, usize) + Send + Sync + 'static,
{
    ...
    if config.debug {
        println!("🕐 Starting GStreamer frame extraction for: {:?}", video_path);
        println!("🕐 Sample rate: {}", sample_rate);
    }
    ...
    if config.debug {
        ...
        println!("🕐 Breakdown: Duration={:.2}%, Pipeline={:.2}%, Streaming={:.2}%", 
            duration_time.as_secs_f64() / total_extraction_time.as_secs_f64() * 100.0,
            pipeline_time.as_secs_f64() / total_extraction_time.as_secs_f64() * 100.0,
            streaming_time.as_secs_f64() / total_extraction_time.as_secs_f64() * 100.0);
    }
...
fn create_optimized_pipeline_v2(video_path: &Path, _sample_rate: f64) -> Result<gst::Pipeline> {
    ...
    println!("🚀 Creating OPTIMIZED GStreamer pipeline v2 for: {}", absolute_path);
...
fn create_seek_based_pipeline(video_path: &Path) -> Result<gst::Pipeline> {
    ...
    println!("🎯 Creating SEEK-BASED GStreamer pipeline for: {}", absolute_path);
...
fn create_extraction_pipeline(video_path: &Path, sample_rate: f64, config: &Config) -> Result<gst::Pipeline> {
    // Use absolute path for GStreamer
    let absolute_path = video_path.canonicalize()
        .unwrap_or_else(|_| video_path.to_path_buf())
        .to_string_lossy()
    ...
    // Add elements to pipeline
    pipeline.add_many([&filesrc, &decodebin, &videoconvert, &videoscale, &videorate, &appsink])?;
...
fn buffer_to_rgb_image(buffer: &gst::BufferRef) -> Result<RgbImage> {
    let map = buffer.map_readable().map_err(|_| anyhow::anyhow!("Failed to map buffer"))?;
    ...
    if data.len() != expected_size as usize {
        return Err(anyhow::anyhow!("Unexpected buffer size: {} vs {}", data.len(), expected_size));
    }
...
pub fn get_video_duration_gstreamer(video_path: &Path, config: &Config) -> Result<f64> {
    ...
    let duration = info.duration().unwrap_or(gst::ClockTime::ZERO).seconds_f64();
    if config.debug {
...
pub fn get_video_info_gstreamer(video_path: &Path) -> Result<crate::analyzer::VideoInfo> {
    ...
    let video_info = video_streams.first()
        .ok_or_else(|| anyhow::anyhow!("No video stream found"))?;
    
    Ok(crate::analyzer::VideoInfo {
        duration: info.duration().unwrap_or(gst::ClockTime::ZERO).seconds_f64(),
        width: video_info.width(),
        height: video_info.height(),
        fps: video_info.framerate().numer() as f64 / video_info.framerate().denom() as f64,
        bitrate: Some(info.audio_streams().first()
            .map(|s| s.bitrate() as u64).unwrap_or(0)),
    })
}
...
mod tests {
    ...
    fn test_pipeline_creation() {
        ...
        let result = create_extraction_pipeline(video_path, sample_rate);
        assert!(result.is_ok());
    }
        ...
        if video_path.exists() {
            ...
            match result {
                Ok(video_info) => {
                    ...
                    println!("Video info: duration={:.2}s, size={}x{}, fps={:.2}", 
    ...
    fn test_gstreamer_duration_extraction() {
        let video_path = PathBuf::from("tests/samples/downscaled/27.mkv");
        if video_path.exists() {
            let result = get_video_duration_gstreamer(&video_path);
            assert!(result.is_ok());
