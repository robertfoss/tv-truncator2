...
pub fn cut_video_segments_gstreamer(
    ...
    let input_absolute = input_path.canonicalize()
        .unwrap_or_else(|_| input_path.to_path_buf())
        .to_string_lossy()
        .to_string();
    let output_absolute = output_path.canonicalize()
    ...
    for (i, (start_time, end_time)) in sorted_segments.iter().enumerate() {
        println!("🕐 Processing segment {}: {:.2}s - {:.2}s", i + 1, start_time, end_time);
...
fn create_cutting_pipeline(input_path: &str, output_path: &str) -> Result<gst::Pipeline> {
    ...
    pipeline.add_many([
        ...
        &videorate, &x264enc, &matroskamux, &filesink
    ])?;
...
mod tests {
    ...
    fn test_build_segments_to_keep_multiple_removals() {
        ...
        assert_eq!(segments, vec![(0.0, 10.0), (20.0, 30.0), (40.0, 50.0), (60.0, 100.0)]);
    }
