...
use super::common::{AlgorithmComparison, ComparisonMetrics, CommonSegment};
...
pub fn compare_algorithms(
    ...
    let ssim_features_avg_confidence = if ssim_features_count > 0 {
        ssim_features_results.iter().map(|s| s.confidence).sum::<f64>() / ssim_features_count as f64
    } else {
        0.0
    };
...
pub fn print_algorithm_comparison(comparison: &AlgorithmComparison) {
    ...
    println!("  Average confidence: {:.1}%", comparison.metrics.multi_hash_avg_confidence * 100.0);
    println!("  Processing time: {:?}", comparison.metrics.processing_time_multi_hash);
    
    println!("\nSSIM+Features Algorithm:");
    println!("  Segments found: {}", comparison.metrics.ssim_features_count);
    println!("  Average confidence: {:.1}%", comparison.metrics.ssim_features_avg_confidence * 100.0);
    println!("  Processing time: {:?}", comparison.metrics.processing_time_ssim_features);
    
    println!("\nAgreement:");
    println!("  Segments found by both: {}", comparison.metrics.agreement_count);
    println!("  Multi-Hash only: {}", comparison.multi_hash_only.len());
    println!("  SSIM+Features only: {}", comparison.ssim_features_only.len());
    ...
    if !comparison.agreement.is_empty() {
        ...
        for (i, segment) in comparison.agreement.iter().enumerate() {
            ...
            println!("  {}. {:.1}s - {:.1}s (duration: {:.1}s, confidence: {:.1}%)",
                     i + 1, segment.start_time, segment.end_time, duration, segment.confidence * 100.0);
        }
    }
    ...
    let speed_ratio = if comparison.metrics.processing_time_ssim_features.as_secs_f64() > 0.0 {
        comparison.metrics.processing_time_multi_hash.as_secs_f64() / 
        comparison.metrics.processing_time_ssim_features.as_secs_f64()
    } else {
        1.0
    };
    ...
    println!("\nPerformance:");
    if speed_ratio > 1.0 {
        println!("  Multi-Hash is {:.1}x faster than SSIM+Features", speed_ratio);
    } else {
        println!("  SSIM+Features is {:.1}x faster than Multi-Hash", 1.0 / speed_ratio);
    }
    ...
    if detection_ratio > 1.0 {
        println!("  Multi-Hash found {:.1}x more segments than SSIM+Features", detection_ratio);
    } else {
        println!("  SSIM+Features found {:.1}x more segments than Multi-Hash", 1.0 / detection_ratio);
    }
    
    let agreement_rate = if comparison.metrics.multi_hash_count + comparison.metrics.ssim_features_count > 0 {
        (comparison.metrics.agreement_count as f64 * 2.0) / 
        (comparison.metrics.multi_hash_count + comparison.metrics.ssim_features_count) as f64
    } else {
        0.0
    };
