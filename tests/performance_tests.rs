...
fn test_gstreamer_frame_extraction_performance() {
    ...
    assert!(gstreamer_duration.as_secs() < 120, "GStreamer processing took too long: {:?}", gstreamer_duration);
}
...
fn test_gstreamer_memory_usage() {
    ...
    assert_eq!(temp_files.len(), 0, "GStreamer should not create temporary files");
}
...
fn test_quick_mode_performance() {
    ...
    println!("Quick mode improvement: {:.1}%", 
             (normal_duration.as_secs_f64() - quick_duration.as_secs_f64()) / normal_duration.as_secs_f64() * 100.0);
}
...
fn test_synthetic_samples_performance() {
    ...
    for dir in synthetic_dirs {
        ...
        assert!(duration.as_secs() < 60, "Synthetic sample {} took too long: {:?}", dir, duration);
    }
}
...
fn test_error_handling_performance() {
    ...
    assert!(duration.as_secs() < 5, "Error handling took too long: {:?}", duration);
}
...
fn test_large_file_performance() {
    ...
    assert!(duration.as_secs() < 300, "Large file processing took too long: {:?}", duration);
}
