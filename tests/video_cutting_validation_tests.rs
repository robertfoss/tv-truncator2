...
#[test]
#[ignore = "Optional local *.mkv under synthetic/downscaled_2file; duration-removed window is brittle vs SimilarityAlgorithm::Current (run manually with media present)"]
fn test_cutting_removes_correct_duration_downscaled_2file() -> Result<()> {
    use std::fs;

    // Use downscaled_2file test samples
    let input_dir = PathBuf::from("tests/samples/synthetic/downscaled_2file");
    ...
    if video_files.len() < 2 {
        println!(
            "Skipping test_cutting_removes_correct_duration_downscaled_2file — need >=2 local .mkv files (large media is gitignored; CI keeps segments.json only)"
        );
        return Ok(());
    }
    ...
    println!(
        "Original durations: {:.2}s, {:.2}s",
        original_duration_01, original_duration_02
    );
    ...
    let completed_count = processors
        .iter()
        .filter(|p| matches!(p.state, tvt::state_machine::ProcessingState::Done { .. }))
        .count();
    ...
    println!(
        "Truncated durations: {:.2}s, {:.2}s",
        truncated_duration_01, truncated_duration_02
    );
    ...
    assert!(
        removed_01 >= 28.0 && removed_01 <= 35.0,
        "Should remove ~31.8s from file 01, but removed {:.2}s",
        removed_01
    );
    assert!(
        removed_02 >= 28.0 && removed_02 <= 35.0,
        "Should remove ~31.8s from file 02, but removed {:.2}s",
        removed_02
    );

    // Verify truncated files are shorter than originals
    assert!(
        truncated_duration_01 < original_duration_01,
        "Truncated file 01 should be shorter than original"
    );
    assert!(
        truncated_duration_02 < original_duration_02,
...
fn test_truncated_files_are_valid_videos() -> Result<()> {
    ...
    let config = Config {
        ...
        similarity_threshold: 0.75,
        similarity_algorithm: tvt::similarity::SimilarityAlgorithm::Current,
        audio_algorithm: tvt::AudioAlgorithm::Chromaprint,
        dry_run: false,
        quick: false,
        verbose: false,
        debug: false,
    ...
    if video_files.len() < 2 {
        println!(
            "Skipping test_truncated_files_are_valid_videos — need >=2 local .mkv under synthetic/downscaled_2file"
        );
        return Ok(());
    }
    video_files.truncate(2);
    ...
    for video_file in &video_files {
        ...
        if truncated_path.exists() {
            ...
            assert!(
                duration > 0.0,
                "Truncated file should have positive duration"
            );

            println!(
                "Truncated file is valid: {} ({:.2}s)",
                truncated_path.file_name().unwrap().to_string_lossy(),
                duration
            );
        }
    }
...
fn test_single_segment_cutting() -> Result<()> {
    ...
    if video_files.len() < 2 {
        println!(
            "Skipping test_single_segment_cutting — need >=2 local .mkv under synthetic/downscaled_2file"
        );
        return Ok(());
    }
    video_files.truncate(2);
    ...
    let total_segments: usize = processors
        .iter()
    ...
    assert!(
        total_segments > 0,
        "Should detect at least one common segment"
    );
